...
const Tablas = ({ title, subtitle, loading, datas, columns, caption, inputs, Accions, showOptions, lang, icon, sort, print, datasPrint }) => {
  ...
  if (!columns) {
    console.warn("Missing props 'columns'");
    return;
  }
  if (!datas) {
    console.warn("Missing props 'datas[]'");
    return;
  }
  if (!Accions) {
    console.warn("Missing props 'Accions'");
    return;
  }
    ...
    <>
      ...
      <Box borderRadius={3} boxShadow={6} padding={2} bgcolor="background.paper">
        ...
        {loading ? (
          ...
          <TableContainer id="table_print" className={style.tcontainer}>
                ...
                <TableRow className={style.trtitles}>
                  {columns.map((col, index) => (
                    <TableCell align={col.align ? col.align : "left"} key={index}>
                      {sort?.desc && (
                        <span onClick={() => sort.desc(col.field)} className={style.arrow}>
                          ↓
                        </span>
                      )}
                      {col.title}
                      {sort?.asc && (
                        <span onClick={() => sort.asc(col.field)} className={style.arrow}>
                          ↑
                        </span>
                      )}
                    </TableCell>
                  ))}
                  <TableCell align="center">{lang ? lang.opciones : "Opciones"}</TableCell>
                </TableRow>
              </TableHead>
                  ...
                  <TableRow hover key={index} className={style.tableRow}>
                    {columns.map((column, i) => (
                      <TableCell key={i} className={style.tableCell} align={column.align ? column.align : "left"}>
                        <span className={style.columntitleSpan}>{column.title}:</span>
                        <span style={column.style ? column.style : column.styleCondition ? column.styleCondition[data[column.styleFieldCondition]] : null}>
                          {column.before ?? null}
                          {column.isNumber
