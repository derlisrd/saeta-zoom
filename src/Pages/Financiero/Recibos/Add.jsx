...
function Add() {
  ...
  const agregar = async () => {
    ...
    if (lista.length > 12) {
      setError({ active: true, msg: "El recibo solo tiene hasta 12 items" });
      return false;
    }
    setError({ active: false, msg: "" });
    setLoading(true);
    let res = await APICALLER.get({
      ...
      where: `nro_factura,=,'${nro.value}',and,estado_factura,=,1`,
      fields: "id_cliente,nombre_cliente,ruc_cliente,total_factura,nro_factura,id_factura,fecha_factura,estado_factura",
    });
  ...
  return (
    <Dialog open={dialogs.add} onClose={close} fullWidth maxWidth="md">
      ...
      <DialogContent>
        <Grid container spacing={2}>
          ...
          <Grid item xs={12} sm={6}>
            <Button onClick={agregar} variant="contained">
              AGREGAR
            </Button>
          </Grid>
          <Grid item xs={6} sm={6}>
