...
function BuscarProductos() {
        ...
        if (id) {
            ...
            if (res.response) {
                ...
                if (categoriaID === '4') {
                    ...
                    setRangos({ esferico: new_rangos_esferico, cilindrico: new_rangos_cilindrico, adicion: [], bases: [] })

                    new_rangos_esferico.forEach(RE => {
                        let cil = []
                        let total = 0;
                        new_rangos_cilindrico.forEach(RC => {
                            found = res.results.find(ele => String(ele.graduacion_esferico) === RE && String(ele.graduacion_cilindrico) === RC);
                            if (found) {
                    ...
                    setRangos({ adicion: new_rangos_adicion, bases: new_rangos_base, esferico: [], cilindrico: [] })
                    new_rangos_adicion.forEach(RA => {
                        let bas = []
                        let total = 0;
                        new_rangos_base.forEach(RB => {
                            found = res.results.find(ele => (String(ele.base) === RB.base && String(ele.lado) === RB.lado && String(ele.adicion) === RA));
    ...
    useEffect(() => {
        const timer = setTimeout(async () => {
            if (search !== '') {
                setLoading(true)
                let where = `tipo_producto,=,1`;
                if (categoriaID !== '') {
                    where += `,and,id_categoria_producto,=,${categoriaID}`;
                }
                let res = await APICALLER.get({
                    table: "productos",
    ...
    return (<Grid container spacing={2}>
        ...
        <Grid item xs={12} sm={3}>
            <SelectDeposito opciones={depositos} name='deposito_id' value={depositoID} onChange={e => { setDepositoID(String(e.target.value)) }} />
        </Grid>
        <Grid item xs={12} sm={3}>
            <SelectCategoria opciones={categorias} name='id_categoria_producto' value={categoriaID} onChange={e => { setCategoriaID(String(e.target.value)) }} />
        </Grid>
        {
