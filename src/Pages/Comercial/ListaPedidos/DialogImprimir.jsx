...
function DialogImprimir() {
  ...
  const getLista = useCallback(async () => {
    if (dialogs.imprimir) {
      ...
      let [fact, items, receta] = await Promise.all([
        APICALLER.get({
          table: "pedidos",
          include: "users,clientes",
          on: "id_user,user_id_pedido,id_cliente,cliente_id_pedido",
          where: `id_pedido,=,${formSelect.id_pedido}`,
          fields:
            "id_cliente,tipo_pedido,armazon_id,total_pedido,nombre_cliente,ruc_cliente,direccion_cliente,nombre_user,fecha_pedido,estado_pedido,id_pedido,obs_cliente,obs_laboratorio,codigo_cliente_pedido",
        }),
  ...
  return (
    <Dialog open={dialogs.imprimir} onClose={atras} fullWidth maxWidth="xs">
      <DialogContent>
        {loading ? (
          <LinearProgress />
        ) : (
          <div id="print_2" ref={divRef}>
            <table className="table_pedido" width="100%">
              <tbody>
                <tr>
                  <td>
                    <h1> {dataEmpresa.nombre_empresa}</h1>
                  </td>
                </tr>
                <tr>
                  <td>
                    <h1>PEDIDO NRO: {factura.datos.id_pedido} - USO INTERNO</h1>
                  </td>
                </tr>
                <tr>
                  <td align="center">
                    <h3>TIPO DE PEDIDO: {tipoPedidos[factura.datos.tipo_pedido]} </h3>
                  </td>
                </tr>
                <tr>
                  <td>FECHA: {factura.datos.fecha_pedido}</td>
                </tr>
                <tr>
                ...
                <tr>
                  <td>
                    DOC: {factura.datos.ruc_cliente} COD: {factura.datos.id_cliente}
                  </td>
                </tr>
                <tr>
                  <td>CLIENTE: {factura.datos.nombre_cliente} </td>
                </tr>
                <tr>
                  <td>CODIGO CLIENTE: {factura.datos.codigo_cliente_pedido} </td>
                </tr>
                <tr>
                  <td>DIRECCION: {factura.datos.direccion_cliente} </td>
                </tr>
              </tbody>
            </table>
              ...
              <tbody>
                ...
                <tr>
                  <td align="right" colSpan={4}>
                    TOTAL: {funciones.numberFormat(factura.datos.total_pedido)}
                  </td>
                </tr>
              </tbody>
            </table>
            ...
            <table className="table_pedido" border="1">
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td width="20%">
                    <b>{factura.receta.dnp_izquierdo}</b>
                  </td>
                </tr>
                <tr>
                  <td width="20%">
            ...
            <table className="table_pedido" border="1">
              <tbody>
                <tr>
                  <td>ARMAZON: {factura.armazon}</td>
                </tr>
                <tr>
                  <td>OBS LABORATORIO: {factura.datos.obs_laboratorio}</td>
                </tr>
                <tr>
                  <td>OBS CLIENTE: {factura.datos.obs_cliente}</td>
                </tr>
              </tbody>
            </table>
          </div>
      ...
      <DialogActions>
        <Button color="info" variant="outlined" onClick={atras} size="large">
          {" "}
          CERRAR{" "}
        </Button>
        <Button color="success" variant="contained" onClick={imprimir} size="large">
          {" "}
          IMPRIMIR{" "}
        </Button>
      </DialogActions>
