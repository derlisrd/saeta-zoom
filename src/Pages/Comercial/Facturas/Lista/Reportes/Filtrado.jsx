...
function Filtrado({ setLista, setLoading, setTotal }) {
  ...
  const filtrar = async () => {
    ...
    let res = await APICALLER.get({
      ...
      where,
      fields: `fecha_factura,nro_factura,ruc_cliente,nombre_cliente,tipo_factura,total_factura,estado_factura`,
    });
    if (res.response) {
      console.log(res.results);
      let arr = [],
        tipos = { 1: "Contado", 2: "Credito" },
        total = 0;
      res.results.forEach((elm) => {
        arr.push({ ...elm, condicion: tipos[elm.tipo_factura], total_factura: elm.estado_factura === "1" ? parseFloat(elm.total_factura) : parseFloat(elm.total_factura) * -1 });
        if (elm.estado_factura === "1") {
          total += parseFloat(elm.total_factura);
        }
      });
      arr.push({ fecha_factura: "", nro_factura: "", ruc_cliente: "", nombre_cliente: "", tipo_factura: "", estado_factura: "", total_factura: total });
      setTotal(total);
      setLista(arr);
    } else {
      console.log(res);
    ...
    <Grid container spacing={1}>
      ...
      <Grid item xs={12} sm={3}>
        <Autocomplete
          ...
          noOptionsText="No existe en registro..."
      ...
      <Grid item xs={12} sm={2}>
        <Stack direction="row" spacing={1}>
          <Button onClick={filtrar} variant="outlined">
            Filtrar
          </Button>
